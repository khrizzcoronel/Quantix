...
**Credencial de acceso:** `qpcp vwew idgk hizb` (Nota: en la implementación de código, esto se inyectará siempre mediante variables de entorno `.env`, **NUNCA** hardcodeado en el código fuente).
...
## 4. Tareas a Ejecutar
- [ ] **TASK-012-01:** Implementar clase `EmailSender` (basada en `smtplib` / `email.message`) configurada con puerto 587/465, leyendo host, correo y la app password (`qpcp vwew idgk hizb`) desde `.env`.
