...
**Módulo:** 012 / 014 - Análisis Estadístico & Reportes Avanzados (`/analisis`)  
**Nivel Organizacional (Anthony):** Estratégico y Táctico (C-Level & Supervisión)  
**Fecha de Publicación:** Septiembre 2026  
**Estándar de referencia:** Adaptado de IEEE 830 / ISO/IEC/IEEE 29148  
**Componentes Frontend:** `Analisis.tsx`, `ReportePersonalizadoBuilder.tsx`, `ReportePreview.tsx`  
...
## 2. Matriz de Control de Acceso (RBAC) y Sincronización Reactiva

| Perfil de Usuario | Nivel de Acceso | Alcance de Sucursales | Capacidades Específicas |
| :--- | :---: | :---: | :--- |
| **DIRECTOR** | Pleno | **Global (Multi-sucursal)** | Acceso irrestricto a métricas consolidadas de todas las sucursales o filtrado individual mediante selector interactivo. Creación y borrado de plantillas públicas y privadas. Conmutación reactiva instantánea en tiempo real. |
...
### Comportamiento de Sincronización Reactiva:
* Cuando el usuario `DIRECTOR` conmuta la sede en el Header (`useSucursalStore`), el módulo `Analisis.tsx` recalcula y vuelve a consultar inmediatamente todos los datasets (KPIs, tendencias, ABC, RFM, heatmap y reportes) sin recargar la página.
* Para el usuario `SUPERVISOR`, el selector de sucursal se bloquea con candado visual fijado en `current_user.sucursal_id` y el backend rechaza con HTTP 403 cualquier intento de override foráneo.
...
| **RF-REP-04** | Segmentación Algorítmica RFM | **MUST** | El motor analítico debe clasificar a los clientes en 9 segmentos estratégicos (Campeones, Leales, Potenciales, En Riesgo, Inactivos, Nuevos, Prometedores, etc.) basados en Recencia, Frecuencia y Valor Monetario calculados mediante `NTILE(5)`. |
...
| **RF-REP-09** | Fila Fija de Totales y Paginación | **MUST** | La tabla de resultados (`ReportePreview.tsx`) debe incluir ordenamiento por cabecera, paginación configurable (25, 50, 100 registros) y una fila inferior fija y destacada con los totales acumulados (unidades, subtotal, descuentos, impuestos, total facturado y margen). |
| **RF-REP-10** | Motor de Plantillas de Reporte Persistentes | **MUST** | El sistema permite a los usuarios memorizar configuraciones personalizadas en base de datos PostgreSQL (`plantillas_reporte`, Alembic 0010) distinguiendo plantillas públicas del sistema y privadas del usuario, recuperándolas en un solo clic. |
               ...
               usuarios, sucursales, pagos_venta
                ...
                [ DuckDB Capa Gold (Dimensional) ]
                  ...
                  └── gold.fact_pagos   (Trazabilidad por Método de Cobro)
                              │
                              ▼
         [ FastAPI Router: /api/v1/reportes ] ◄── Control RBAC (HTTP 403)
                              │
                              ▼
             [ Frontend React 19: Analisis.tsx ]
             ├── ReportePersonalizadoBuilder.tsx
             └── ReportePreview.tsx
```
...
1. `GET /api/v1/reportes/analisis/kpis-avanzados`
   - Parámetros: `fecha_desde`, `fecha_hasta`, `sucursal_id`.
...
7. `GET /api/v1/reportes/columnas-disponibles`
   - Salida: `CatalogoColumnasResponse` (15 columnas configurables).
8. `POST /api/v1/reportes/generar`
   - Payload: `GenerarReporteRequest` (columnas, filtros, agrupacion, orden, limite, offset).
   - Salida: `ReporteGeneradoResponse`.
9. `GET /api/v1/reportes/plantillas` | `POST` | `DELETE /{id}`
   - CRUD completo de plantillas de reporte con persistencia relacional en PostgreSQL (`plantillas_reporte`).
